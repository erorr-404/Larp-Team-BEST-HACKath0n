...
namespace System
{
    ...
    internal static class HalfHelper
    {
        ...
        private static ushort GetBits(Half half)
        {
            return MemoryMarshal.Cast<Half, ushort>(MemoryMarshal.CreateSpan(ref half, 1))[0];
        }

        private static Half FromBits(ushort bits)
        {
            return MemoryMarshal.Cast<ushort, Half>(MemoryMarshal.CreateSpan(ref bits, 1))[0];
        }

        public static float HalfushortToSingle(ushort value)
        {
            uint result = mantissaTable[offsetTable[value >> 10] + (value & 0x3ff)] + exponentTable[value >> 10];
            return FloatToInt.Convert(result);
        }

        public static float HalfToSingle(Half half)
        {
            ushort halfValue = GetBits(half);
        ...
        {
            uint value = FloatToInt.Convert(single);
        ...
        {
            return FromBits((ushort)(GetBits(half) ^ 0x8000));
        }
        public static Half Abs(Half half)
        {
            return FromBits((ushort)(GetBits(half) & 0x7fff));
        }

        public static bool IsNaN(Half half)
        {
            return ((GetBits(half) & 0x7fff) > 0x7c00);
        }
        ...
        {
            return (GetBits(half) == 0x7c00);
        }
        public static bool IsNegativeInfinity(Half half)
        {
            return (GetBits(half) == 0xfc00);
        }
    }
}
